...
/**
 * Controller for the VisionForge Dashboard
 * Handles sidebar navigation, live DB statistics, and animated UI elements.
 */
public class DashboardController {
        ...
        try {
            ...
            if (files.isEmpty()) {
                ...
                for (MediaFile file : files) {
                    activities.add(file.getType() + " " + file.getFileName()
    ...
    private void updateSystemStatus() {
        if (dbStatusLabel != null) dbStatusLabel.setText("DB: Connected ✅");
        if (pluginStatusLabel != null) pluginStatusLabel.setText("Plugins: 4 Loaded");
        if (themeStatusLabel != null) themeStatusLabel.setText("Theme: Light");
    }
    ...
    // ---------- Navigation Handlers ----------
    @FXML private void onRefreshClicked(ActionEvent event) {
    ...
    @FXML private void openHome(ActionEvent event) {
        System.out.println("[Dashboard] Home clicked");
        switchScene(event, "/fxml/home_view.fxml");
    }

    @FXML private void onHomeClicked(ActionEvent event) { openHome(event); } // alias fix
    @FXML private void onProfileClicked(ActionEvent event) { openProfile(event); } // alias fix
